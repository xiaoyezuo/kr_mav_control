...
class SO3CmdToMavrosTester
{
 public:
  ...
  void attitude_raw_callback(const mavros_msgs::AttitudeTarget::SharedPtr &msg);
  void odom_callback(const geometry_msgs::PoseStamped::SharedPtr &odom);
  ...
  mavros_msgs::AttitudeTarget::SharedPtr attitude_raw_msg_;
  nav_msgs::Odometry::SharedPtr odom_msg_;
  std::mutex mutex;
  ...
  private:
    ...
    rclcpp::Publisher<kr_mav_msgs::SO3Command>::SharedPtr so3_cmd_pub_;
    rclcpp::Publisher<nav_msgs::Odometry>::SharedPtr odom_pub_;
    rclcpp::Publisher<sensor_msgs::Imu>::SharedPtr imu_pub_;
    rclcpp::Subscription<mavros_msgs::AttitudeTarget>::SharedPtr attitude_raw_sub_;
    rclcpp::Subscription<geometry_msgs::PoseStamped>::SharedPtr odom_pose_sub_;
    std::vector<kr_mav_msgs::SO3Command> so3_cmd_msgs_;
    std::vector<nav_msgs::Odometry> odom_msgs_;
    std::vector<sensor_msgs::Imu> imu_msgs_;
};
 ...
 */
SO3CmdToMavrosTester::SO3CmdToMavrosTester(): Node("so3_to_mavros_tester")
{
...
//   odom_pose_sub_ = nh.subscribe("mavros/vision_pose/pose", 5, &SO3CmdToMavrosTester::odom_callback, this, ros::TransportHints().tcpNoDelay());
    so3_cmd_pub_ = this->create_publisher<kr_mav_msgs::SO3Command>("so3_cmd", 5);
    odom_pub_ = this->create_publisher<nav_msgs::Odometry>("odom", 5);
    imu_pub_ = this->create_publisher<sensor_msgs::Imu>("mavros/imu/data", 5);
    attitude_raw_sub_ = this->create_subscription<mavros_msgs::AttitudeTarget>("mavros/setpoint_raw/attitude", 5, std::bind(&SO3CmdToMavrosTester::attitude_raw_callback, this, std::placeholders::_1));
    odom_pose_sub_ = this->create_subscription<geometry_msgs::PoseStamped>("mavros/vision_pose/pose", 5, std::bind(&SO3CmdToMavrosTester::odom_callback, this, std::placeholders::_1));
}
...
{
  //enable_motors = false
  auto so3_cmd = kr_mav_msgs::SO3Command();
  so3_cmd.force.x = 0.0;
  so3_cmd.force.y = 0.0;
  so3_cmd.force.z = 0.0;
